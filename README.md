...
OV7670 카메라의 내부 레지스터를 설정하기 위한 Custom IP
MicroBlaze-V가 AXI4-Lite Register에 카메라 설정값을 저장하면, IP 내부의 `sccb_ctrl` 모듈이 SCCB 통신을 통해 OV7670 카메라에 설정값을 전송
...
| Address  | Register         | 설명                                         |
| -------- | ---------------- | ------------------------------------------ |
| `0x00`   | Control / Status | 카메라 초기화 시작 신호, 설정할 레지스터 개수, 초기화 완료 상태를 제어하고 확인하는 레지스터 |
| `0x04 ~` | Config Register  | OV7670 카메라에 전달할 레지스터 주소와 설정 데이터를 저장하는 레지스터      |
...
OV7670 카메라에서 들어오는 영상 데이터를 수신하여 BRAM에 저장하는 Custom IP
카메라의 `PCLK`, `VSYNC`, `HREF`, `DATA[7:0]` 신호를 이용해 픽셀 데이터를 읽고, RGB565 형식의 데이터를 RGB444 형식으로 변환
...
BRAM에서 읽어온 픽셀 데이터를 VGA 신호로 변환하여 모니터에 출력하는 Custom IP
`25MHz` Pixel Clock을 기준으로 VGA `640×480 @ 60Hz` 타이밍을 생성하고, `320×240` 카메라 영상을 화면 중앙에 표시
...
BRAM에서 읽은 카메라 픽셀 데이터를 VGA Controller로 보내기 전에 필터 처리하는 Custom IP
MicroBlaze-V가 AXI4-Lite Register에 `filter_sel` 값을 쓰면, 해당 값에 따라 실시간으로 필터 모드가 변경
